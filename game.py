...
enemy_death = pygame.image.load('images/enemy/fog.png').convert_alpha()
enemy_death = pygame.transform.scale(enemy_death, (100, 100))
enemy_list_in_game = []
...
restart_label = label.render('Restart?', True, (232, 137, 218))
restart_label_rect = restart_label.get_rect(topleft=(480, 75))
play_label = label.render('Play', True, (232, 137, 218))
play_label_rect = play_label.get_rect(topleft=(480, 75))
start_label = label.render('Ghost Girl', True, (232, 137, 218))

fireballs_left = 10000
fireball = pygame.image.load('images/fireball.png').convert_alpha()
fireball_boom = pygame.image.load('images/explode (1).png').convert_alpha()  # <--------- вставить картинку взрыва файрболла
fireballs = []
...
while running:
    ...
    if play_label_rect.collidepoint(mouse) and pygame.mouse.get_pressed()[0]: # <---- регистрация нажатия на кнопке
        start = True

    if start:

        screen.blit(backyard, (backyard_x, 0)) # <---- экран (х2)
        ...
        if gameplay:  # основной игровой цикл
            ...
            if enemy_list_in_game: # <---- проверка наличия врага
                for (i, el) in enumerate(enemy_list_in_game):
                    ...
                    screen.blit(enemy, el)  # <---- появление врага
                    enemy_speed_mod = rd.randint(-10,10) # <---- модификатор скорости (выключить при необходимости)
                    el.x -= 5 + enemy_speed_mod

                    if el.x < -50:
                        enemy_list_in_game.pop(i)

                    if player_rect.colliderect(el): # <---- смерть игрока
                        gameplay = False
            keys = pygame.key.get_pressed() # <---- модуль ходьбы и прыжков
            ...
            if is_jump == False:  # <------------------- Анимация прыжка и замедление скорости для увеличенных тиков
                if player_anim_count == 7:
                    ...
                    player_anim_count_0 += (ticks/100)/2
                    player_anim_count = int(player_anim_count_0)

            backyard_x -= 2  # <---- движение заднего фона
            if backyard_x == -700:
                backyard_x = 0
                    ...
                    if enemy_list_in_game:
                        for (index, enemy_el) in enumerate(enemy_list_in_game):
                            if el.colliderect(enemy_el):
                                ...
                                screen.blit(enemy_death, (el.x + 30, el.y - 64))
                                fireballs.pop(i)
